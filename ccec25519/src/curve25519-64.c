...
void fexpand(limb *output, const uint8_t *in)
{
    ...
    x = cc_load64_le(in + 12);
    output[2] = (x >> 6) & 0x7ffffffffffff;

    x = cc_load64_le(in + 19);
    output[3] = (x >> 1) & 0x7ffffffffffff;

    x = cc_load64_le(in + 24);
...
void fcontract(uint8_t *output, const limb *input)
{
    ...
    cc_store64_le((uint64_t) (t[0] | (t[1] << 51)), output);
    cc_store64_le((uint64_t) ((t[1] >> 13) | (t[2] << 38)), output + 8);
    cc_store64_le((uint64_t) ((t[2] >> 26) | (t[3] << 25)), output + 16);
    cc_store64_le((uint64_t) ((t[3] >> 39) | (t[4] << 12)), output + 24);
    /* clang-format on */
}
