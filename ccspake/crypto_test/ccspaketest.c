...
static int test_fixed_point(ccec_const_cp_t cp, size_t seed_len, const uint8_t *seed, cc_unit *order, const cc_unit *xy)
{
    ...
    for (uint16_t i = 1; i < 1000; i++) {
        ...
        if (ccec_validate_pub_and_projectify_ws(ws, cp, r, (ccec_const_affine_point_t)s, rng) != CCERR_OK) {
            goto loop;
        }

        // Check that (r * #E) is a point on the curve.
        if (ccec_mult_blinded_ws(ws, cp, s, order, r, rng) != 0 || !ccec_is_point_ws(ws, cp, s)) {
            goto loop;
        }
...
const struct p256_sha256_hkdf_test_vector {
    ...
    const char *sk;
    const char *aad;
} p256_sha256_hkdf_test_vectors[] = {
#include "../test_vectors/p256-sha256-hkdf.kat"
};

const size_t p256_sha256_hkdf_test_vectors_num =
    CC_ARRAY_LEN(p256_sha256_hkdf_test_vectors);
...
static int test_kat()
{
    ...
    for (unsigned i = 0; i < p256_sha256_hkdf_test_vectors_num; i++) {
        const struct p256_sha256_hkdf_test_vector *test =
            &p256_sha256_hkdf_test_vectors[i];
        ...
        byteBuffer tv_Y = hexStringToBytes(test->Y);
        byteBuffer tv_Z = hexStringToBytes(test->Z);
        byteBuffer tv_V = hexStringToBytes(test->V);
        byteBuffer tv_mac_a = hexStringToBytes(test->mac_a);
        byteBuffer tv_mac_b = hexStringToBytes(test->mac_b);
        byteBuffer tv_sk = hexStringToBytes(test->sk);
        byteBuffer tv_aad = hexStringToBytes(test->aad);
        ...
        is(ccspake_prover_init(ctx_p, scp, mac, rng, tv_aad->len, tv_aad->bytes, tv_w0->len, tv_w0->bytes, tv_w1->bytes), CCERR_OK, "Initialize SPAKE2+ prover");
        is(ccspake_verifier_init(ctx_v, scp, mac, rng, tv_aad->len, tv_aad->bytes, tv_w0->len, tv_w0->bytes, sizeof(L), L), CCERR_OK, "Initialize SPAKE2+ verifier");
        ...
        is(ccn_read_uint(n, ccspake_ctx_XY_x(ctx_p), 32, tv_X->bytes + 1), CCERR_OK, "Reading x(X) failed");
        is(ccn_read_uint(n, ccspake_ctx_XY_y(ctx_p), 32, tv_X->bytes + 1 + 32), CCERR_OK, "Reading y(X) failed");

        // Override y and Y = G * y.
        is(ccn_read_uint(n, ccspake_ctx_xy(ctx_v), tv_y->len, tv_y->bytes), CCERR_OK, "Reading y failed");
        is(ccn_read_uint(n, ccspake_ctx_XY_x(ctx_v), 32, tv_Y->bytes + 1), CCERR_OK, "Reading x(Y) failed");
        is(ccn_read_uint(n, ccspake_ctx_XY_y(ctx_v), 32, tv_Y->bytes + 1 + 32), CCERR_OK, "Reading y(Y) failed");
        ...
        uint8_t Z[65] = { 0x04, 0x00 };
        uint8_t V[65] = { 0x04, 0x00 };

        // Check shared secret Z.
        ccn_write_uint_padded(n, ccspake_ctx_Z_x(ctx_p), 32, Z + 1);
        ccn_write_uint_padded(n, ccspake_ctx_Z_y(ctx_p), 32, Z + 1 + 32);
        ok_memcmp(Z, tv_Z->bytes, tv_Z->len, "Z ≠ Z_tv");

        ccn_write_uint_padded(n, ccspake_ctx_Z_x(ctx_v), 32, Z + 1);
        ccn_write_uint_padded(n, ccspake_ctx_Z_y(ctx_v), 32, Z + 1 + 32);
        ok_memcmp(Z, tv_Z->bytes, tv_Z->len, "Z ≠ Z_tv");

        // Check shared secret V.
        ccn_write_uint_padded(n, ccspake_ctx_V_x(ctx_p), 32, V + 1);
        ccn_write_uint_padded(n, ccspake_ctx_V_y(ctx_p), 32, V + 1 + 32);
        ok_memcmp(V, tv_V->bytes, tv_V->len, "V ≠ V_tv");

        ccn_write_uint_padded(n, ccspake_ctx_V_x(ctx_v), 32, V + 1);
        ccn_write_uint_padded(n, ccspake_ctx_V_y(ctx_v), 32, V + 1 + 32);
        ok_memcmp(V, tv_V->bytes, tv_V->len, "V ≠ V_tv");

        uint8_t mac_p[tv_mac_a->len], mac_v[tv_mac_a->len];
        ...
        // Check MACs.
        ok_memcmp(mac_p, tv_mac_a->bytes, tv_mac_a->len, "MAC_a ≠ MAC_a_tv");
        ok_memcmp(mac_v, tv_mac_b->bytes, tv_mac_b->len, "MAC_b ≠ MAC_b_tv");
...
int ccspake_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    is(test_kat(), 0, "KAT tests failed");
