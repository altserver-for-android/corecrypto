...
static int ccspake_lazy_gen_xy_XY_ws(cc_ws_t ws, ccspake_ctx_t ctx)
{
    ...
    ccec_pub_ctx_t pub = (ccec_pub_ctx_t)CC_ALLOC_WS(ws, ccec_pub_ctx_ws(n));
    ...
    if ((rv = ccec_projectify_ws(ws, cp, U, ccspake_ctx_MN(ctx, !ccspake_ctx_is_prover(ctx)), rng))) {
        goto cleanup;
    }
...
static int ccspake_kex_process_ws(cc_ws_t ws, ccspake_ctx_t ctx, size_t y_len, const uint8_t *y)
{
    ...
    ccec_pub_ctx_t Q_pub = (ccec_pub_ctx_t)CC_ALLOC_WS(ws, ccec_pub_ctx_ws(n));
    ccec_ctx_init(cp, Q_pub);

    ccec_projective_point *MN = CCEC_ALLOC_POINT_WS(ws, n);
    ...
    if ((rv = ccec_projectify_ws(ws, cp, MN, ccspake_ctx_MN(ctx, ccspake_ctx_is_prover(ctx)), rng))) {
        goto cleanup;
    }

    // S = w0 * M (prover) or S = w0 * N (verifier)
    if ((rv = ccec_mult_blinded_ws(ws, cp, S, ccspake_ctx_w0(ctx), MN, rng))) {
        goto cleanup;
    }
    ...
    if (ccspake_ctx_is_prover(ctx)) {
        ...
        if ((rv = ccec_validate_pub_and_projectify_ws(ws, cp, L, (ccec_const_affine_point_t)ccspake_ctx_L(ctx), rng))) {
            goto cleanup;
        }
