...
static int ccecies_gcm_kat_negative_test(const struct ccecies_vector *test, int test_counter)
{
    ...
    status_decrypt_gcm = test_ccecies_decrypt_gcm(remote_key,
                                                  ...
                                                  &output_size,
                                                  plaintext_bis->bytes);
    // Corrupted public key may become invalid (not on curve) which should make the decrypt function return CCERR_INTERNAL, or
    // remains valid and should make decrypt function return CCMODE_INTEGRITY_FAILURE. Both return codes are tested.
    ok_or_goto(
        (status_decrypt_gcm == CCMODE_INTEGRITY_FAILURE || status_decrypt_gcm == CCERR_INTERNAL), "Corrupted public key", errout);
