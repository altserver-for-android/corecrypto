...
def rekey (root_key, iv):
      zIV = iv[:]
      zIV[15] &= 0x7F
      keyGenCipher = Cipher(algorithms.AES(bytes(root_key)), modes.ECB(), backend=default_backend())
...
def one_go_test(root_key, ad, iv, plaintext, tag_length):
    (t,c) = siv_hmac_enc(root_key, ad, iv, plaintext, tag_length)
    result = siv_hmac_dec(root_key, ad, iv, c, t, tag_length)
    if (result==plaintext):
        print "{"
        print "\t.keyStr = \"" + root_key + "\","
        ha2s(ad)
        print "\t.init_ivStr = \"" + iv + "\","
...
root_key = b'01020304050607080102030405060708f1f2f3f4f5f6f7f8f1f2f3f4f5f6f7f8'

def perform_testsdef():
    one_go_test(root_key, [],'','plaintext only test', 20)
    one_go_test(root_key, [], b'a1a2a3a4', 'plaintext only test', 20)
    one_go_test(root_key, [b'b1b2b3b4'], b'a1a2a3a4', 'plaintext only test', 20)
    one_go_test(root_key, [b'1234',b'ff00ff00'],b'a1a2a3a4a5a6a7a8a9','text to encrypt', 20)
