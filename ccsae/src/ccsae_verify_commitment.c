...
int ccsae_verify_commitment(ccsae_ctx_t ctx, const uint8_t *peer_commitment)
{
    ...
    cc_require(ccec_mult_blinded(cp, I1, ccsae_ctx_peer_commitscalar(ctx), PWE, rng) == CCERR_OK, cleanup);

    // [WPA3] 12.4.5.4: PWE = I1 + PEER_CE
    cc_require(ccec_full_add(cp, PWE, I1, PEER_CE) == CCERR_OK, cleanup);

    // [WPA3] 12.4.5.4: I2 = rand * PWE
    cc_require(ccec_mult_blinded(cp, I2, ccsae_ctx_rand(ctx), PWE, rng) == CCERR_OK, cleanup);
    ...
    cc_require(cczp_add(zq, context, ccsae_ctx_commitscalar(ctx), ccsae_ctx_peer_commitscalar(ctx)) == CCERR_OK, cleanup);
