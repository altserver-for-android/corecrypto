...
// Helper function to allow WORKSPACE_N() function generation for ccsrp_generate_client_S_ws().
CC_INLINE int ccsrp_client_divmod_ws(cc_ws_t ws, cc_size n, cc_unit *tmp1, const cc_unit *tmp2)
{
    return ccn_mod_ws(ws, n, tmp1, 2 * n, tmp1, n, tmp2);
}

CC_PURE cc_size CCSRP_CLIENT_DIVMOD_WORKSPACE_N(cc_size n)
{
    return CCN_DIV_EUCLID_WORKSPACE_N(2 * n, n);
}
                               ...
                               const cc_unit *B)
{
    ...
    if (tmp1_bitlen >= ccdh_gp_prime_bitlen(ccsrp_ctx_gp(srp))) {
        ...
        status = ccsrp_client_divmod_ws(ws, n, tmp1, tmp2);
