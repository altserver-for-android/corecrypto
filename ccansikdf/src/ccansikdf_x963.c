...
#define ccansikdf_x963_entry(_ctx_, _di_, _num_) (                               \
    (ccdigest_ctx_t)(                                                            \
        (uint8_t *)_ctx_->dis + ccansikdf_x963_padded_entry_size(_di_) * (_num_) \
    )                                                                            \
)
...
void ccansikdf_x963_final(const struct ccdigest_info *di, ccansikdf_x963_ctx_t ctx, void *key)
{
    ...
    for (size_t i = 0; i < r; i++) {
        ccdigest_final(di, ccansikdf_x963_entry(ctx, di, i), out);
    ...
    cc_memcpy(out, digest, ctx->klen - r * di->output_size);
}
