/* Copyright (c) (2011,2012,2015-2021) Apple Inc. All rights reserved.
 *
...
{
    int status;
    CC_DECL_WORKSPACE_OR_FAIL(ws,CCN_DIV_EUCLID_WORKSPACE_N(na, nd));
...
{
    return nd + 1 + 4 + CCN_DIV_USE_RECIP_WORKSPACE_N(na, nd);
}
...
{
    return 6 * CC_MAX_EVAL(2 * nd, na) + 4 + na;
}
...
int ccn_div_use_recip_ws(cc_ws_t ws, cc_size nq, cc_unit *q, cc_size nr, cc_unit *r, cc_size na, const cc_unit *a, cc_size nd, const cc_unit *d, const cc_unit *recip_d)
{
    ...
    cc_unit *d1 = CC_ALLOC_WS(ws,1+CC_MAX_EVAL(nd,n-nd_actual));  /* d1[1+CC_MAX(nd,n-nd_actual)] */
    ...
    ccn_setn(CC_MAX_EVAL(nd+1,n-nd_actual+1), d1, nd, d);
    ccn_setn(n, a1, na, a);
    ccn_zero(na, q1);
    ...
    for (size_t k=0; k<loop_iterations; k++) {
        ...
        n1 = CC_MAX_EVAL(2*nd, n1-nd_actual+1); // adjust n for performance
    }
    ...
    // 0 <= r < d.
    cc_assert(ccn_cmpn(n, a1, 1+CC_MAX_EVAL(nd,n-nd_actual), d1) < 0);
