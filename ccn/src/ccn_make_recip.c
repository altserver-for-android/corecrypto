...
{
    return 6 * (n + 1) + CC_MAX_EVAL(
        CCN_MUL_WORKSPACE_N(n + 1), CCN_SQR_WORKSPACE_N(n + 1));
}
...
void ccn_make_recip_ws(cc_ws_t ws, cc_size d_nunits, cc_unit *recip, const cc_unit *d)
{
    ...
    // Locals
    ccn_setn(n, tmpd, CC_MIN_EVAL(d_nunits, n), d);
    ...
    for (size_t i = 0; i < S; i++) {
        size_t k = CC_MIN_EVAL((b + 1), 1 + R0_PRECISION * (((size_t)1) << (i + 1)));
