...
ccaes_ios_hardware_common_init(int operation CC_UNUSED, ccaes_hardware_aes_ctx_t ctx, size_t rawkey_len, const void *rawkey)
{
    ...
    rawkey_len = ccaes_key_length_to_nbytes(rawkey_len);
                            ...
                            const void *in, void *out, size_t nblocks)
{
	...
	while (remaining) {
        ...
        uint64_t counter = cc_load64_be(aesRequest.iv.ivBytes+8); // Get the lowest part of the counter
        ...
        if (isOverflow) {
            counter = cc_load64_be(aesRequest.iv.ivBytes);
            counter--;
