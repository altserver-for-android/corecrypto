...
static void cchpke_labeled_extract(cchpke_const_params_t params,
                                  bool kem_suite_id,
                                  size_t salt_nbytes,
                                  const uint8_t *salt,
                                  size_t label_nbytes,
                                  ...
                                  uint8_t *output)
{
    ...
    cchkdf_extract(params->kdf->hashFunction(), salt_nbytes, salt, labeled_ikm_bytes_written, labeled_ikm, output);
                                     ...
                                     uint8_t *shared_secret)
{
    ...
    cchpke_labeled_extract(params, true, 0, NULL, sizeof(HPKE_EAE_LABEL), HPKE_EAE_LABEL, dh_len, dh, eae_prk);
...
cchpke_kem_x25519_generate_key_pair(struct ccrng_state *rng, size_t sk_nbytes, uint8_t *sk, size_t pk_nbytes, uint8_t *pk)
{
    ...
    cccurve25519_make_key_pair(rng, public_key, secret_key);
                                                 ...
                                                 uint8_t *enc)
{
    ...
    cccurve25519(DH, skE, pkR);
    int result = cchpke_kem_x25519_serialize(enc_nbytes, pkE, enc);
    if (result != CCERR_OK) {
        return result;
    }
                                          ...
                                          const uint8_t *pskID)
{
    ...
    cchpke_labeled_extract(params, false, 0, NULL, sizeof(HPKE_PSK_ID_LABEL), HPKE_PSK_ID_LABEL, pskID_nbytes, pskID, pskID_hash);

    uint8_t *info_hash = key_schedule_context + sizeof(mode) + params->kdf->Nh;
    cchpke_labeled_extract(params, false, 0, NULL, sizeof(HPKE_INFO_LABEL), HPKE_INFO_LABEL, info_nbytes, info, info_hash);

    // secret = Extract(shared_secret, "secret", psk)
    uint8_t secret[CCHPKE_KDF_EXTRACT_MAX_SIZE];
    cchpke_labeled_extract(params,
                           false,
    ...
    int ret = cchpke_labeled_expand(params,
                          false,
                          params->kdf->Nh,
                          secret,
