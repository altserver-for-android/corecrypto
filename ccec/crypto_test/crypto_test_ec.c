...
static void ccec_cczp_mod_prime_tests(ccec_const_cp_t cp)
{
    ...
    for (size_t i = 0; i < 1000; i++) {
        ...
        cczp_to(zp, a, a);
        cczp_to(zp, b, b);
        is(cczp_mul(zp, r, a, b), CCERR_OK, "cczp_mul() failed");
        cczp_from(zp, r, r);
...
static void ccec_key_sizes_tests()
{
...
static void ccec_scalar_tests()
{
    ...
    // R == O ?
    ok(ccn_is_zero(n, ccec_point_z(R, cp)), "R = 0 * G = O");
    ...
    // R == O ?
    ok(ccn_is_zero(n, ccec_point_z(S, cp)), "R = 7 * O = O");
    ...
    rv = ccec_mult_inner_ws(ws, cp, R, s, ccn_bitsof_n(n), G);
    ...
    ok_ccn_cmp(n, ccec_point_x(R, cp), S2, "R = (q-1 mod 2^255) * G");
    ...
    ok_ccn_cmp(n, ccec_point_x(R, cp), S3, "R = (q-1 mod 2^254) * G");
...
const struct ccec_mult_edge_case_test_vector {
    ...
    {
        // If x(Q) = x(mask * S) = 0, increment the mask so that x(Q) ≠ 0.
        .s = { CCN256_C(15,59,0c,30,8b,73,02,b8,db,f9,2e,e2,39,4b,a4,89,6e,92,68,91,2b,cf,b2,6a,e1,d0,3e,2e,ef,56,d7,53) },
        .r = { CCN256_C(86,df,92,96,8f,36,67,f8,8b,f2,ee,18,93,f5,0a,7f,29,db,3c,85,5f,fa,ae,69,a3,04,95,e7,bc,57,23,3c) },
        .mask = { 0x00, 0x00, 0x00, 0x00 },
        .qm = 2
    },
    {
        // If x(Q) = x(mask * S) = 0, increment the mask so that x(Q) ≠ 0.
        // Ensure that overflows when incrementing are handled properly.
        .s = { CCN256_C(75,4d,4c,77,e5,36,c2,99,fe,c5,0c,8d,2e,b8,6e,ea,f7,ca,09,0b,88,d4,c8,61,6a,ac,46,dc,91,6f,d7,a3) },
        .r = { CCN256_C(e8,c8,38,f3,5e,6f,fc,d2,8b,3a,12,78,ad,76,f8,94,aa,04,47,b3,33,c2,d7,12,17,58,18,7b,9a,cd,4e,04) },
        .mask = { 0xff, 0xff, 0xff, 0xff },
        .qm = 2
    },
    {
        // Intermediate result Q, such that x(Q) = x(b * S) = 0.
        .s = { CCN256_C(27,24,ac,95,ba,46,72,14,3a,2a,4b,7a,06,14,da,9b,ff,54,fe,14,4e,7c,77,a4,a4,02,ee,ff,a6,3a,6b,ad) },
        .r = { CCN256_C(7f,e5,ee,1f,e1,8b,a3,67,51,a7,78,29,d5,01,a7,37,c2,61,b7,0a,ee,15,c7,5f,36,f7,86,25,12,e0,03,67) },
        .mask = { 0x00, 0x00, 0x00, 0x00 },
        .qm = 2
    },
    {
        // Intermediate result R, such that x(R) = x(mask * a * S) = 0.
        .s = { CCN256_C(96,b2,f1,82,c1,16,f3,ea,eb,ef,38,f9,97,d8,a1,a8,5d,16,aa,6c,4e,6c,8b,7c,6d,e4,aa,fc,57,28,9f,f0) },
        .r = { CCN256_C(02,bd,8e,bd,06,a3,14,d2,e7,59,d1,6b,34,72,44,9f,4d,5b,d0,b0,7d,26,2c,d6,76,19,74,a5,47,ce,ee,b1) },
        .mask = { 0x00, 0x00, 0x00, 0x00 },
        .qm = 2
    },
    {
        // Compute R = s * G = (q - 5) * G.
        // As s = 0 (mod mask), where mask = 0x80a382e4,
        // one partial result of the blinded scalar multiplication will be
        // the point at infinity as we're multiplying by zero, Q = 0 * S.
        .s = { CCN256_C(6b,17,d1,f2,e1,2c,42,47,f8,bc,e6,e5,63,a4,40,f2,77,03,7d,81,2d,eb,33,a0,f4,a1,39,45,d8,98,c2,96) },
        .r = { CCN256_C(51,59,0b,7a,51,51,40,d2,d7,84,c8,56,08,66,8f,df,ef,8c,82,fd,1f,5b,e5,24,21,55,4a,0d,c3,d0,33,ed) },
        .mask = { 0xe4, 0x82, 0xa3, 0x00 },
        .qm = 5
    },
    {
        // Compute R = s * S = (q - 8) * S where x(S) = 0.
        // As (s >> 1) = 0 (mod mask), where mask = 0x83095609,
        // one partial result of the blinded scalar multiplication will be
        // the point at infinity as we're multiplying by zero, Q = 0 * P.
        .s = { CCN256_C(00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00,00) },
        .r = { CCN256_C(12,6c,91,34,ee,e3,b9,79,e0,2d,08,62,dd,07,cd,24,a1,f4,ab,e8,a7,f1,86,0a,6c,07,e1,c1,93,06,a2,8b) },
        .mask = { 0x09, 0x56, 0x09, 0x03 },
        .qm = 8
    }
};
...
static int ccec_mult_edge_case_tests(void)
{
    ...
    for (size_t i = 0; i < ccec_mult_edge_case_test_vectors_num; i++) {
        ...
        int rv = ccec_affine_point_from_x_ws(ws, cp, (ccec_affine_point_t)R, tv->s);
        is(rv, CCERR_OK, "ccec_affine_point_from_x() failed");

        rv = ccec_projectify_ws(ws, cp, S, (ccec_const_affine_point_t)R, global_test_rng);
        is(rv, CCERR_OK, "ccec_projectify() failed");

        ccrng_sequence_init(&seq_rng, 4, tv->mask);

        cc_unit s[n];
        ccn_sub1(n, s, cczp_prime(ccec_cp_zq(cp)), tv->qm);

        rv = ccec_mult_blinded_ws(ws, cp, R, s, S, rng);
        is(rv, CCERR_OK, "ccec_mult_blinded_ws() failed");

        rv = ccec_affinify_ws(ws, cp, (ccec_affine_point_t)R, R);
        is(rv, CCERR_OK, "ccec_affinify() failed");

        ok_ccn_cmp(n, ccec_point_x(R, cp), tv->r, "R = s * S");
...
static void ccec_pairwise_consistency_tests(void)
{
    ...
    is(ccec_pairwise_consistency_check(key, global_test_rng), false,
        "ccec_pairwise_consistency_check() should've failed");

    isnt(ccecdh_pairwise_consistency_check(key, NULL, global_test_rng), CCERR_OK,
        "ccecdh_pairwise_consistency_check() should've failed");
...
{
    int ntests = 0;
    ntests += 8001 * 10; // ccec_random_tests
#if CCN_MULMOD_224_ASM
    ntests += 8001 * 2;  // ccec_random_tests
#endif
#if CCN_MULMOD_256_ASM
    if (ccec_cp_256_asm_if_available()) {
        ntests += 8001;  // ccec_random_tests
    }
#endif
