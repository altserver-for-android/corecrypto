                               ...
                               ccec_pub_ctx_t pub_out)
{
    ...
    int result = ccec_validate_pub_and_projectify_ws(ws, cp, P_p, (ccec_const_affine_point_t)ccec_ctx_point(pub), rng);
    if (result != CCERR_OK) {
        goto cleanup;
    }
                                          ...
                                          ccec_full_ctx_t unblinding_key)
{
    ...
    ccec_pub_ctx_t P = (ccec_pub_ctx_t)CC_ALLOC_WS(ws, ccec_pub_ctx_ws(n));
