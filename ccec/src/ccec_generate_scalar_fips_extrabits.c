...
{
    return n + ENTROPY_NUNITS_MAX +
        CCN_DIV_EUCLID_WORKSPACE_N(ENTROPY_NUNITS_MAX, n);
}
                                           ...
                                           cc_unit *k)
{
    ...
    cc_require((retval=ccn_mod_ws(ws, n, k, ccn_nof_size(entropy_len), kn, n, qm1))==0,errOut);
