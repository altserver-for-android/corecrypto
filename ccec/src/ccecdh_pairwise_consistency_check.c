                                      ...
                                      struct ccrng_state *rng)
{
    ...
    int rv = ccec_projectify_ws(ws, cp, B, base, rng);
    if (rv) {
        goto out;
    }
                                                ...
                                                struct ccrng_state *rng)
{
    ...
    int rv = ccecdh_fast_scalar_mult_ws(ws, cp, R, ccec_ctx_k(full_key), base, rng);
    if (rv) {
        goto out;
    }
                                                ...
                                                struct ccrng_state *rng)
{
    ...
    int rv = ccecdh_fast_scalar_mult_ws(ws, cp, R, d, base, rng);
    if (rv) {
        goto out;
    }
...
{
#if CC_SMALL_CODE
    return ccec_make_pub_from_priv_ws(ws, cp, rng, ccec_ctx_k(key), base, ccec_ctx_pub(key));
#else
    return ccecdh_fast_compute_pub_from_priv_ws(ws, cp, key, base, rng);
#endif
}
...
{
#if CC_SMALL_CODE
    return ccecdh_compute_shared_secret_ws(ws, key, pub, sk_len, sk, rng);
#else
    return ccecdh_fast_compute_shared_secret_ws(ws, ccec_ctx_cp(key), ccec_ctx_k(key), (ccec_const_affine_point_t)ccec_ctx_point(pub), sk, rng);
#endif
}

// Override workspace definitions so they're correct for default and CC_SMALL_CODE=1.
#pragma workspace-override ccecdh_pcc_compute_pub_from_priv_ws ccec_make_pub_from_priv_ws
#pragma workspace-override ccecdh_pcc_compute_pub_from_priv_ws ccecdh_fast_compute_pub_from_priv_ws
#pragma workspace-override ccecdh_pcc_compute_shared_secret_ws ccecdh_compute_shared_secret_ws
#pragma workspace-override ccecdh_pcc_compute_shared_secret_ws ccecdh_fast_compute_shared_secret_ws
                                         ...
                                         struct ccrng_state *rng)
{
    ...
    ccec_full_ctx_t reference_key = (ccec_full_ctx_t)CC_ALLOC_WS(ws, ccec_full_ctx_ws(n));
    ...
    int rv = ccecdh_pcc_compute_pub_from_priv_ws(ws, cp, reference_key, base, rng);
    ...
    rv = ccecdh_pcc_compute_shared_secret_ws(ws, reference_key, ccec_ctx_pub(full_key), &shared_key2_size, shared_key2, rng);
    cc_require(rv == CCERR_OK, errOut);
