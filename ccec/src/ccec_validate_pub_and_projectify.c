...
int ccec_validate_pub_and_projectify(ccec_const_cp_t cp,
                                     ...
                                     struct ccrng_state *masking_rng)
{
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCEC_VALIDATE_PUB_AND_PROJECTIFY_WORKSPACE_N(ccec_cp_n(cp)));
    int rv = ccec_validate_pub_and_projectify_ws(ws, cp, r, public_point, masking_rng);
