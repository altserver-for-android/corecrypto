...
cc_unit * cc_indexable ccec_ctx_k( ccec_full_ctx_t key) {
    // ccec_full_ctx_size() already accounts for cp and pad so subtract them.
    cc_unit *points = CC_FLEX_BYTE_COUNT_WORKAROUND(key->point->xyz, ccec_full_ctx_size(ccec_ccn_size(key->cp)) - offsetof(struct ccec_full_ctx, point));
    return points + 3 * ccec_ctx_n(key);
}
...
int ccec_generate_key_deterministic(ccec_const_cp_t cp,
                                size_t entropy_len, const uint8_t *cc_counted_by(entropy_len) entropy,
...
CC_NONNULL((2, 3))
void ccec_compact_export(const int fullkey, void *cc_unsafe_indexable out, const ccec_full_ctx_t key);
 ...
 */
CC_NONNULL((1, 2))
void ccec_compact_export_pub(void *cc_unsafe_indexable out, const ccec_pub_ctx_t key);
...
CC_NONNULL((1, 5))
int
ccec_der_export_priv(const ccec_full_ctx_t key, ccoid_t key_oid, int includePublic, size_t out_len, void *cc_sized_by(out_len) out);
