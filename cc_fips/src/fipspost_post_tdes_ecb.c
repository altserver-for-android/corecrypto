...
static int fipspost_post_tdes_ecb_encrypt(uint32_t fips_mode)
{
    uint8_t output[8];
    const struct ccmode_ecb *ecb_mode = ccdes3_ecb_encrypt_mode();
...
{
    int ret_e = fipspost_post_tdes_ecb_encrypt(fips_mode);
