...
int fipspost_post(uint32_t fips_mode, struct mach_header *pmach_header)
{
    ...
    int test_counter = 0;
    int return_on_failure = !FIPS_MODE_IS_FORCEFAIL(fips_mode);
...
#define run_post(post_test, ...)                                                                                                 \
    do {                                                                                                                         \
        ...
        if ((result = post_test(fips_mode, ##__VA_ARGS__)) != 0) {                                                               \
            ...
            debugf("PASSED: (%u ms) - " #post_test, (uint32_t)cc_absolute_time_to_msec((double)(end_time - post_time)));         \
        }                                                                                                                        \
    } while (0);
     ...
     */
    return_on_failure = FIPS_MODE_IS_NOINTEG(fips_mode) ? 0 : !FIPS_MODE_IS_FORCEFAIL(fips_mode);
