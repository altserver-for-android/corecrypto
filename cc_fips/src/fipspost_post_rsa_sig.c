...
const size_t SIGNATURE_LEN = (RSA_NBITS + 7) / 8;
...
CC_INLINE int fipspost_post_rsa_sign(uint32_t fips_mode)
{
    ...
    ccdigest(di, MESSAGE_LEN, MESSAGE, msg_digest);

    uint8_t sig[SIGNATURE_LEN];
    size_t siglen=sizeof(sig);

    // Sign the digest
    if (ccrsa_sign_pkcs1v15(full_key, di->oid, di->output_size, msg_digest,  &siglen, sig)) {
...
CC_INLINE int fipspost_post_rsa_verify(uint32_t fips_mode)
{
    ...
    // Verify Signature
    bool ok;
    if (ccrsa_verify_pkcs1v15(pub_key, ccoid_sha256, CCSHA256_OUTPUT_SIZE, msg_digest, SIGNATURE_LEN, SIGNATURE, &ok)) {
