...
{
    return CCN_DIV_USE_RECIP_WORKSPACE_N(ns, n);
}
...
{
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCZP_MODN_WORKSPACE_N(ns, cczp_n(zp)));
    int rv = cczp_modn_ws(ws, zp, r, ns, s);
...
void cczp_mod_default_ws(cc_ws_t ws, cczp_const_t zp, cc_unit *r, const cc_unit *s2n)
{
    ...
    tmp1 = CC_ALLOC_WS(ws, 2 * n + 2); // tmp1 is 2*n
...
#pragma workspace-override cczp_mod_ws cczp_mod_default_ws
