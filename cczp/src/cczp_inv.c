 ...
 */
static void cczp_inv_update_redc_ws(cc_ws_t ws, cczp_const_t zp, cc_unit *r, const cc_unit *a, cc_unit f, const cc_unit *b, cc_unit g, cc_unit p0inv)
{
    cc_size n = cczp_n(zp);
    CC_DECL_BP_WS(ws, bp);
    ...
    cczp_negate(zp, t0, a);
    ccn_mux(n, f_lt_z, t0, t0, a);
    f = (f ^ -f_lt_z) + f_lt_z;

    // If g < 0, negate g and b.
    cczp_negate(zp, t1, b);
    ccn_mux(n, g_lt_z, t1, t1, b);
    g = (g ^ -g_lt_z) + g_lt_z;
    ...
    t0[n] += ccn_addmul1(n, t0, cczp_prime(zp), (t0[0] * p0inv) & mask_lo);
    ccn_shift_right(n + 1, t0, t0, CCN_UNIT_HALF_BITS - 1);
    ccn_set(n, r, t0);
...
int cczp_inv_default_ws(cc_ws_t ws, cczp_const_t zp, cc_unit *r, const cc_unit *x)
{
    ...
    // Compute -p[0]^(-1) (mod 2^w) for Montgomery's REDC algorithm.
    cc_unit p0inv = -ccn_invert(cczp_prime(zp)[0]);
    ...
    for (size_t i = 0; i < cc_ceiling(iterations, CCN_UNIT_HALF_BITS - 1); i++) {
        ...
        for (size_t j = 0; j < CCN_UNIT_HALF_BITS - 1; j++) {
            ...
            cc_unit u_lt_v = ccop_lt(ua, va) & 1;
        ...
        cczp_inv_update_redc_ws(ws, zp, tmp, a, f0, b, g0, p0inv);
        // b = |f1 * a + g1 * b| >> CCN_UNIT_HALF_BITS-1 (mod p)
        cczp_inv_update_redc_ws(ws, zp, b, a, f1, b, g1, p0inv);
...
#pragma workspace-override cczp_inv_ws cczp_inv_default_ws
