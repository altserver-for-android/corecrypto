...
static void test_cczp_power_fns(void)
{
    ...
    cczp_power_blinded_ws(ws, zp, r, s, e, global_test_rng);
    ...
    isnt(cczp_power_fast_ws(ws, zp, r, t, e), 0, "cczp_power_fast when base > p");
    isnt(cczp_mm_power_fast_ws(ws, zp, r, t, e), 0, "cczp_mm_power_fast when base > p");
    isnt(cczp_power_blinded_ws(ws, zp, r, t, e, global_test_rng), 0, "cczp_power_blinded_ws when base > p");
...
static void test_cczp_power_fns_randomized(cczp_const_t zp)
{
    ...
    for (int i = 0; i < NUM_RANDOM_POWER_TESTS; i++) {
        ...
        is(cczp_power_blinded_ws(ws, zp, r4, b, e, global_test_rng), 0, "cczp_power_blinded_ws randomized");
...
static void test_cczp_sqrt(void)
{
    // 597035519 = 3 mod 4
...
int cczp_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    num_tests += 26;                              // test_cczp_power_fns
    num_tests += 2 * 11 * NUM_RANDOM_POWER_TESTS; // test_cczp_power_fns_randomized
    num_tests += 14 * 2 + 1;                      // test_cczp_sqrt
    num_tests += 2 * (5 * NUM_RANDOM_SQRT_TESTS); // test_cczp_sqrt_randomized
    ...
    num_tests += 5;                               // test_cczp_quadratic_residue
