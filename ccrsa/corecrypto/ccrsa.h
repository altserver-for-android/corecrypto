...
// Compute the internal structure size in bytes based on key (eg modulus) byte size.
// 3072bit RSA key -> 384byte.
...
ccrsa_priv_ctx_t ccrsa_get_private_ctx_ptr(ccrsa_full_ctx_t fk) {
    // Subtract sizeof(struct cczp) because ccrsa_full_ctx_size() accounts for it and we aren't starting at the struct base.
    cc_unit * pb_ccn = CC_FLEX_BYTE_COUNT_WORKAROUND(fk->pb_ccn, ccrsa_full_ctx_size(ccn_sizeof(fk->pb_bitlen)) - sizeof(struct cczp));
    // XXX: is there anything after k that this indexable pointer could allow someone to access?
    return (ccrsa_priv_ctx_t) ((pb_ccn + 3 * ccrsa_ctx_n(fk) + 1) + ccrsa_ctx_n(fk));
}
...
int ccrsa_get_fullkey_components(const ccrsa_full_ctx_t key, uint8_t *cc_unsafe_indexable modulus, size_t *modulusLength,
                                 uint8_t *cc_unsafe_indexable exponent, size_t *exponentLength,
