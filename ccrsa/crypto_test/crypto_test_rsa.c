    ...
    while(run_test)
    {
        switch(test_index)
        {
            ...
            case 0:
                ...
                memset(&tmp[2],0xff,key_nbytes-2);
        ...
        if (run_test)
        {
            ...
            if (ccn_cmp(n,tmp_u,ccrsa_ctx_m(fk))<0) {
                ...
                ok(ccn_write_uint_padded_ct(n, tmp_u, key_nbytes, tmp)>=0,"serializing");

                // Try to decrypt, expected to fail
                decryptedData->len=key_nbytes;
                CC_DECL_WORKSPACE_TEST(ws);
                ok((status = ccrsa_decrypt_eme_pkcs1v15_blinded_ws(ws, global_test_rng, fk,
                                                        &decryptedData->len, decryptedData->bytes,
                                                        key_nbytes,  tmp)) == expected_status,
                   test_description);
                CC_FREE_WORKSPACE(ws);
            }
            else {
...
static int ccrsa_test_make_priv()
{
    ...
    for (size_t i = 0; i < CC_ARRAY_LEN(rsa_makepriv_key_list); i++) {
        const int key_nbits = 2048 + 8;                           // Largest key size we care about
