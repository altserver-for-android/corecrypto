...
static uint8_t FAKE_DIGEST[CCSHA256_OUTPUT_SIZE] = {
...
static int ccrsa_pairwise_consistency_check_ws(cc_ws_t ws,
...
bool ccrsa_pairwise_consistency_check(const ccrsa_full_ctx_t full_key, struct ccrng_state *rng)
{
    ...
    CC_DECL_WORKSPACE_RV(ws, CCRSA_PAIRWISE_CONSISTENCY_CHECK_WORKSPACE_N(n), rv);
    if (rv != CCERR_OK) {
        return false;
    }

    rv = ccrsa_pairwise_consistency_check_ws(ws, full_key, rng);
