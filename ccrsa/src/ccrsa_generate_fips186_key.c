...
static int cczp_find_next_prime(cczp_t p, struct ccrng_state *rng_mr)
{
    cc_size n = cczp_n(p);
    cc_size MR_iterations = ccrsa_num_mr_iterations_aux(ccn_bitlen(n, cczp_prime(p)));
...
cczp_generate_auxiliary_primes(cc_size r1bits, cczp_t r1, cc_size r2bits, cczp_t r2, struct ccrng_state *rng, struct ccrng_state *rng_mr, struct ccrsa_fips186_trace *trace) {
    ...
    if (trace) {
        ...
        trace->bitlen1 = ccn_bitlen(CCZP_N(r1),CCZP_PRIME(r1));
        trace->bitlen2 = ccn_bitlen(CCZP_N(r2),CCZP_PRIME(r2));
...
// Output is {R, r1r2x2}
static int cczp_compute_R(cc_size n, cc_unit *R, cc_unit *r1r2x2, cczp_const_t r1, cczp_const_t r2)
{
    ...
    int rv = cczp_modn(r2, tmp1, cczp_n(r1), cczp_prime(r1));
    if (rv) {
        return rv;
    }

    rv = cczp_inv(r2, tmp1, tmp1);
    ...
    // r1*r2*2
    ccn_mul(r_n, r1r2x2, tmp2, R);
    ccn_cleartop(n, r1r2x2, 2*r_n);
...
static int
ccrsa_generate_probable_prime_from_auxiliary_primes(cc_size pbits, cczp_t p, cc_unit *X,
                                                    ...
                                                    struct ccrsa_fips186_trace *trace)
{
    ...
    cc_unit R[n], tmp[n], r1r2x2[n]; //vla
    ...
    for (size_t ctr=0; (ctr<RESEED_MAX_RETRIES) && (prime_status==CCRSA_KEYGEN_PRIME_NEED_NEW_SEED);ctr++)
    {
        ...
        // 4) Y = X+((R–X) mod 2r1r2)
        ccn_mod(n, CCZP_PRIME(p), n, X, n, r1r2x2);
        // (R-X) mod 2r1r2
        cc_unit borrow = ccn_sub(n, CCZP_PRIME(p), R, cczp_prime(p));
        ccn_add(n, tmp, r1r2x2, cczp_prime(p));
        ccn_mux(n, borrow, CCZP_PRIME(p), tmp, cczp_prime(p));
        // X+((R–X) mod 2r1r2)
        ...
        {
            // 6) Check p >= 2^pbits
            if ((c > 0) || (pbits < ccn_bitlen(n, cczp_prime(p)))) {
            ...
            // 10) p=p+2.r1.r2
            c = ccn_add(n, CCZP_PRIME(p), cczp_prime(p), r1r2x2);
        }
    }
                              ...
                              struct ccrsa_fips186_trace *trace)
{
    ...
    for (int i = 0; ret && (i < GENERATE_PROBABLE_PRIME_MAX_RETRIES); i += 1) {
        ...
        ret = ccrsa_generate_probable_prime_from_auxiliary_primes(pbits, p, X, r1, r2, e_n, e, rng, rng_mr, trace);
    }
                                 ...
                                 struct ccrsa_fips186_trace *trace)
{
    ...
    do {
        ...
        ret = ccrsa_generate_fips186_prime_factors(nbits, p, q, e_n, e, rng, rng_mr, trace);
        cc_require(ret == CCERR_OK, cleanup);

        // Compute m,d and CRT components.
        ret = ccrsa_crt_make_fips186_key(nbits, fk, e_n, e, p, q);
        cc_require(ret == CCERR_OK, cleanup);

        // If d <= 2^pbits, try again.
    } while (ccn_cmpn(n, d, n_pq, dlb) < 1);

    // Ensure the key works.
