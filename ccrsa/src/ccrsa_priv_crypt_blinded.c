    ...
    return 5 * nu + NB_MASK + cczp_nof_n(nu) +
       CC_MAX_EVAL(CCZP_INIT_WORKSPACE_N(nu),
         CC_MAX_EVAL(CCZP_MM_POWER_WORKSPACE_N(nu),
           CC_MAX_EVAL(CCZP_MODN_WORKSPACE_N(n + 1, nu),
             CC_MAX_EVAL(CCZP_MODN_WORKSPACE_N(nu * 2, n),
                                ...
                                const cc_unit *in)
{
    ...
    // Proceed
    status = ccrsa_crt_power_blinded_ws(ws, blinding_rng, fk, out, in);

    // Verify that the computation is correct
    int rc = cczp_mm_power_fast_ws(ws, ccrsa_ctx_zm(fk), tmp, out, ccrsa_ctx_e(fk));
    cc_assert(rc == CCERR_OK); // Sanity check
    rc |= cc_cmp_safe(ccn_sizeof_n(n), tmp, tmp_in);
...
int ccrsa_priv_crypt_blinded(struct ccrng_state *blinding_rng,
                             ...
                             const cc_unit *in)
{
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCRSA_PRIV_CRYPT_BLINDED_WORKSPACE_N(ccrsa_ctx_n(fk)));
    int rv = ccrsa_priv_crypt_blinded_ws(ws, blinding_rng, fk, out, in);
