...
// Helper function to allow WORKSPACE_N() function generation for ccrsa_crt_makekey_ws().
CC_INLINE int ccrsa_crt_divmod_ws(cc_ws_t ws, cc_size pn, cc_unit *dpq, cc_size n, const cc_unit *d, const cc_unit *pm1)
{
    return ccn_mod_ws(ws, pn, dpq, n, d, pn, pm1);
}

CC_PURE cc_size CCRSA_CRT_DIVMOD_WORKSPACE_N(cc_size n)
{
    return CCN_DIV_EUCLID_WORKSPACE_N(n, n);
}
...
int ccrsa_crt_makekey_ws(cc_ws_t ws, ccrsa_full_ctx_t fk)
{
    ...
    /* dp = d mod (p-1) */
    cc_require((status = ccrsa_crt_divmod_ws(ws, pn, ccrsa_ctx_private_dp(fk), n, d, pm1)) == 0, errOut);

    /* dq = d mod (q-1) */
    cc_require((status = ccrsa_crt_divmod_ws(ws, pn, ccrsa_ctx_private_dq(fk), n, d, qm1)) == 0, errOut);
