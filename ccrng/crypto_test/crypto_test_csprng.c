...
#define CCRNG_CSPRNG_TEST_TIMER_NSEC (5 * CC_NSEC_PER_SEC)
#define CCRNG_CSPRNG_TEST_SLEEP_SEC (2 * CCRNG_CSPRNG_TEST_TIMER_NSEC / CC_NSEC_PER_SEC)
#if defined(_WIN32)
#include <windows.h>
static void sleep_some() {
    Sleep(1000 * CCRNG_CSPRNG_TEST_SLEEP_SEC);
}
#else
#include <unistd.h>
static void sleep_some() {
    sleep(CCRNG_CSPRNG_TEST_SLEEP_SEC);
}
#endif
...
static bool csprng_test_one(const struct ccrng_csprng_hmac_drbg_test_vector *tv, struct ccrng_csprng_state *rng, bool use_sleep)
{
    ...
    if (use_sleep && CCRNG_CSPRNG_TEST_TIMER_ENABLED) {
        diag("Sleeping for %d seconds", CCRNG_CSPRNG_TEST_SLEEP_SEC);
        sleep_some();
        diag("Sleep complete\n");
    } else {
...
#if CCRNG_CSPRNG_TEST_TIMER_ENABLED
    int64_t time_delta = (int64_t)(cc_uptime_nsec() - *timer);
    if (time_delta >= (int64_t)CCRNG_CSPRNG_TEST_TIMER_NSEC) {
        *timer = cc_uptime_nsec();
        return true;
    }
#else
    (void) reseed_ctx;
...
#if CCRNG_CSPRNG_TEST_TIMER_ENABLED
    ccrng_csprng_test_timer = cc_uptime_nsec();
#endif
