...
{
    // Sanity check, prime needs to be odd.
...
CC_NONNULL((1, 2, 5, 7, 11)) CC_WARN_RESULT
                                      ...
                                      size_t l_bits, int *warning)
{
    *warning = CCERR_OK;
    ...
    if (q) {
        ...
        if (rv == 0) {
            cc_log_fault("No order given for custom, non-safe DH prime");
            *warning = CCDH_GP_NONSAFE_PRIME;
        }
    }
                            ...
                            size_t l_bits)
{
    ...
    int rv = ccdh_init_gp_from_bytes_ws(ws, gp, n, p_nbytes, p, g_nbytes, g, q_nbytes, q, l_bits, &warning);
