...
static void testDecodeBitstringNullDER() {
    // Test that the result is NULL if der is NULL
    uint8_t end;
    const uint8_t *string = NULL;
    size_t string_len = 0;
...
static void testDecodeOID() {
    ...
    is(ccder_decode_oid(&oid, NULL, &end), NULL, "ccder_decode_oid OID-only der");
...
static void testEncodeEckeyRoundtrip() {
    uint8_t der_buf[80];
...
int ccder_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    int ntests = 0;
    for (struct ccder_test_entry *entry = &tests[0]; entry != ONE_PAST_THE_END(tests); ++entry) {
